...
evidence_policy:
  ...
  title_gap: "标题直接说诊断局限，但真正价值是为整个突变检测分支设定「天花板」——防止阳性突变结果被过度解读为确诊，与 src-fip-022 突变效用研究形成对照"
  evidence_boundary: "限制性研究设计，无法量化替代方案的优势；具体局限性（敏感性/特异性/样本类型/假阳性来源）需阅读全文；不否定突变检测的支持性价值，仅限定其诊断角色上限"
  tension_with:
