...
|------------|---------|------|--------|
| **Data ingest — 文字** | `raw/papers/`, `raw/regulations/` 在用；**1414 paper source cards** + 14 regulation，覆盖 **8 病种**（新增 Cancer 111 张）；V2 enhanced fields 已在所有 cancer cards 添加 | 原始 PDF / web clipper 自动化仍弱，很多 source card 是手工/LLM 管理后的卡片，不是完整 source-file archive | **MEDIUM** |
| **Data ingest — 图片** | CKD 有 8 张已验证非 `candidate-*` 图片/表格资产，FIP/HCM/IBD/Diabetes/Cancer 各有验证图片；vision pipeline 可用 | Obesity 图片仍较少；部分病种图片覆盖不均 | **MEDIUM** |
| **Compile** | `topics/`, `entities/` 已有；topic pages 跨 8 病种；`scripts/compile_trigger.py` 可检测变更并输出 recompile queue；git hook 和 launchd 已激活 | 触发器已建并接入 git hook 和 launchd | **LOW** |
| **Source card 密度** | **1414 paper cards**；CKD/FIP/HCM/IBD/FCV 各有 24+ full deep-extracted；Diabetes 121 cards (29 full + 92 partial)；Obesity 95 cards (7 full + 88 partial)；**Cancer 111 cards (73 full + 10 partial + 28 missing depth)** | 普通 source-card thickening 完成；下一步是 full-text / official-source / image-table precision，而不是补卡 | **MEDIUM** |
...
- Source base now reflects **8 diseases**: CKD, FIP, HCM, IBD, Diabetes, FCV, Obesity, and **Cancer** (new).
- Source count is **1428 cards**: 1414 papers + 14 regulations.
- **Cancer module complete**: 111 source cards, 73 `extraction_depth: full`, all 111 cards now have V2 enhanced fields (`study_design`, `core_argument`, `implicit_premise`, `unexpected_finding`, `title_gap`, `evidence_boundary`).
- **Mis-ingested papers identified**: 6 cancer cards flagged as human studies incorrectly included (src-cancer-098 duplicate, src-cancer-103 FELINE trial, src-cancer-104 Feline Wolf Net, src-cancer-106 canine, src-cancer-110/111 Fer protein).
- **V2 enhanced fields schema**: New fields added under `evidence_policy` for deeper literature presentation—enables Research Mode to display core arguments, evidence boundaries, and unexpected findings instead of template-based summaries.
- **Health checks**: 113 tests passing, all checks PASS.
- Per-disease card counts: CKD (197), FIP (242), HCM (226), IBD (126), Diabetes (121), FCV (296), Obesity (95), Cancer (111).
...
| V2 enhanced fields 扩展 | Cancer 111 cards 已完成；其他 7 病种尚未添加 V2 字段 | 下一步是 HCM pilot (5 篇) 验证 Research Mode 显示效果后决定是否全量推广 | MEDIUM |
| Cancer 误收录清理 | 6 张 cards 标记为人类研究（需移除或重分类） | 低优先级，不影响正常查询 | LOW |
| non-CKD images | CKD 有 8 个 verified assets；其他病种图片覆盖不均 | 更多病种图片需要逐步补充 | MEDIUM |
| Obesity Tier 2 sources | 95 cards，7 deep-extracted (Tier 1 complete) | 如需 intervention detail，可继续 Tier 2 management sources | MEDIUM |
...
来自 `system/indexes/source-depth-map.md` 的 2026-06-21 现实：

1. **V2 Enhanced Fields 验证**
   - Cancer 111 cards 已全部添加 V2 字段
   - 下一步：HCM pilot (5 篇) 验证 Research Mode 显示效果
   - 目标：让文献推荐能勾起 researcher 阅读兴趣

2. **API-first live acceptance**
...
### 3. Evidence-depth gap: 8 病种已有分层厚度

**体现在哪里：**

- CKD: 197 cards (37 full, 160 partial), 35 `deep_extracted`
- FIP: 242 cards (31 full, 211 partial), 31 `deep_extracted`
- HCM: 226 cards (32 full, 194 partial), 32 `deep_extracted`
- IBD: 126 cards (25 full, 101 partial), 25 `deep_extracted`
- FCV: 296 cards (26 full, 270 partial), 26 `deep_extracted`
- Diabetes: 121 cards (29 full + 92 partial), 28 `deep_extracted`
- Obesity: 95 cards (7 full + 88 partial), 5 `deep_extracted`
- **Cancer: 111 cards (73 full + 10 partial + 28 missing depth), 41 `deep_extracted`**
- **1414 paper source cards** 覆盖 8 病种

**为什么重要：**

Q&A 的质量上限不是模型，而是可加载证据的厚度。现在 8 个病种 source-card 基础已完成，Cancer 作为最新模块已有完整 V2 字段覆盖。下一层瓶颈变成 full-text、官方来源、图片/表格和输出特定精度。
...
5. V2 enhanced fields 验证：Cancer 已完成，HCM pilot (5 篇) 下一步。
...
5. 测试套件：python3 scripts/test_query.py（113/113 pass 为 2026-06-21 基线）。
...
8. 8 病种架构：CKD (197), FIP (242), HCM (226), IBD (126), Diabetes (121), FCV (296), Obesity (95), Cancer (111)。
9. V2 enhanced fields：Cancer 111 cards 全部已添加 study_design/core_argument/implicit_premise/unexpected_finding/title_gap/evidence_boundary 字段。
10. 误收录标记：6 张 cancer cards 标记为人类研究（Fer 蛋白、FELINE 试验等命名陷阱）。
```
