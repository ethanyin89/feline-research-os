---
id: out-ckd-dossier-20260408-v1-working-en
type: output
output_kind: dossier
language: en
topic: ckd
...
# Feline CKD Internal Dossier V1
...
The disease should currently be modeled as a progressive fibrotic kidney disorder in which `renal fibrosis / tubulointerstitial fibrosis` is the strongest mechanism backbone supported by the current literature set. Several routinely used clinical variables are not just monitoring conveniences, they are linked back to structural or progression-relevant disease features. That matters because it means the endpoint system should not be built as a purely operational checklist.
...
The treatment layer is usable, but uneven. `Renal diet` has the clearest support in the current source set. A number of other interventions are clearly part of practice, but current evidence support is weaker or mixed. Future treatment outputs should therefore label evidence strength instead of treating all therapies as equally grounded.

The regulatory layer is now good enough for orientation, not submission planning. China, FDA, EMA, and VMD all present plausible pathways, but none of the current official sources justify a shortcut narrative. The right internal question is not “which country is easiest,” but “which jurisdiction fits the intended product type and the evidence package we can realistically generate.”

This file is the working-English dossier. It should not be mistaken for a bilingual or Chinese-facing dossier. The paired `-en` and `-zh` files are derivative language-facing outputs built on top of this working layer.
...
- The most valuable next densification move is to deepen more SDMA-focused studies and stronger treatment primary studies before making the treatment layer more product-decision-ready.
...
- Regulatory section is official-source based, but still route-level, not indication-specific.
- No jurisdiction-specific renal-indication efficacy guidance has yet been ingested.
- Naming note: this file is intentionally the working-English dossier and is separate from the current `-en` and `-zh` derivative dossiers.
